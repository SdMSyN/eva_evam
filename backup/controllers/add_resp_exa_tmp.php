<?php
    ...
    echo $idUser.'--'.$idExam.'--'.$idPreg.'--'.$tipoResp.'--'.$resp;
    $ban = true;
    $msgErr = '';
    if($resp != null){//si hay algo en la respuesta haz algo, si no nada
        $sqlGetRespTmp = "SELECT id FROM $tExaTmp "
                . "WHERE alumno_id='$idUser' AND examen_id='$idExam' AND exa_info_asig_id='$idExamAsig' AND pregunta_id='$idPreg' AND tipo_resp_id='$tipoResp' ";
        ...
        if($resGetIdRespTmp->num_rows > 0){ //si existe la respuesta, actualiza
            ...
            if($tipoResp == 1){
                $sqlInsertRespTmp = "INSERT INTO $tExaTmp (alumno_id, examen_id, exa_info_asig_id, pregunta_id, tipo_resp_id, respuesta, created) "
                        . "VALUES ('$idUser', '$idExam', '$idExamAsig', '$idPreg', '$tipoResp', '$resp', '$dateNow')";
            }else if($tipoResp == 2){
                $sizeCheck = count($resp);
                $respCheck = '';
                for($i = 0; $i < $sizeCheck; $i++){
                ...
                $sqlInsertRespTmp = "INSERT INTO $tExaTmp (alumno_id, examen_id, exa_info_asig_id, pregunta_id, tipo_resp_id, respuesta, created) "
                        . "VALUES ('$idUser', '$idExam', '$idExamAsig', '$idPreg', '$tipoResp', '$respCheck', '$dateNow')";
            }else if($tipoResp == 3){
                $idText = $_POST['idText'];
                $sqlInsertRespTmp = "INSERT INTO $tExaTmp (alumno_id, examen_id, exa_info_asig_id, pregunta_id, tipo_resp_id, respuesta_id, respuesta, created) "
                        . "VALUES ('$idUser', '$idExam', '$idExamAsig', '$idPreg', '$tipoResp', '$idText', '$resp', '$dateNow')";
            }else if($tipoResp == 4){
                $idText = $_POST['idText'];
                $sqlInsertRespTmp = "INSERT INTO $tExaTmp (alumno_id, examen_id, exa_info_asig_id, pregunta_id, tipo_resp_id, respuesta_id, respuesta, created) "
                        . "VALUES ('$idUser', '$idExam', '$idExamAsig', '$idPreg', '$tipoResp', '$idText', '$resp', '$dateNow')";
