<?php
    ...
    $reload = '../views/est_view_exam_details.php?idExam='.$idExam.'&idExamAsig='.$idExamAsig.'$idUser='.$idUser;
    ...
    //echo $idPregSess; 
    $result = '<table>';
    $sqlGetInfo = "SELECT id, nombre, archivo, tipo_resp, valor_preg FROM $tExaPregs WHERE exa_info_id='$idExam' LIMIT $offset, $per_page ";
    ...
    while($rowGetInfo = $resGetInfo->fetch_assoc()){
        ...
        $sqlGetRespEst = "SELECT *, (SELECT nombre FROM $tExaResps WHERE id=$tExaResultPregs.respuesta) as nombreResp FROM $tExaResultPregs "
